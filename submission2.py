...
def encrypt_file(filename, method="aes", key=None):
    ...
    if method == "aes":
        ...
        iv = os.urandom(8)  # DES requires an 8-byte IV
        cipher = Cipher(algorithms.TripleDES(key), modes.CFB(iv), backend=default_backend())  # Consider using AES instead of TripleDES
    elif method == "blowfish":
        iv = os.urandom(8)  # Blowfish uses an 8-byte IV, but consider using a different algorithm
    ...
    return encrypted_filename  # Return only the filename
...
def hash_file_route(file_id, hash_type):
    ...
    if file and file.user_id == current_user.id:
        ...
        flash(f"Hash ({hash_type.upper()}): {file_hash}")
    else:
        flash('File not found')
    return redirect(url_for('file_upload'))
...
def encrypt_file_route(file_id, enc_type):
    ...
    if file and file.user_id == current_user.id:
        ...
        encrypted_filename = encrypt_file(file_path, enc_type, key)
        ...
        flash(f"File encrypted using {enc_type.upper()}. New file: {encrypted_filename}")
        return redirect(url_for('file_upload'))
    else:
        flash('File not found')
        return redirect(url_for('file_upload'))
